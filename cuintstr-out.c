...
void intstrmap_init (intstrmap *self) {
	...
	(*self).mListPos = 0;
	_tmp0_ = g_new0 (sizeof(intstr), DEFAULT_CAPACITY);
...
static void intstrmap_ensureCapa (intstrmap *self) {
	...
	_tmp1_ = capalen;
	(*self).mListData = g_renew (sizeof(intstr), (*self).mListData, capalen);
...
void intstrmap_delkv (intstrmap *self, guint ukey) {
	...
	if (_tmp3_ != -1) {
		...
		_tmp5_ = g_new0 (sizeof(intstr), _tmp4_);
		nlists = _tmp5_;
...
static intstr* _vala_array_dup1 (intstr* self, int length) {
	intstr* result;
	int i;
	result = g_new0 (sizeof(intstr), length);
	for (i = 0; i < length; i++) {
		intstr _tmp0_ = {0};
